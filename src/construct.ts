...
export class OriginVerify extends Construct implements IVerification {
  ...
  constructor(scope: Construct, id: string, props: OriginVerifyProps) {
    ...
    const arn = this.resolveOriginArn(props.origin);
    this.associate(acl, arn);
  }
  ...
  private resolveOriginArn(origin: Origin): string {
    ...
    addError(
      this,
      'Invalid origin: Must be either Api Gateway IStage, IApplicationLoadBalancer, AppSync CfnGraphQLApi, HTTP API, or Lambda Function URL',
    );
    throw new Error('Invalid origin: Must be either Api Gateway IStage, IApplicationLoadBalancer, AppSync CfnGraphQLApi, HTTP API, or Lambda Function URL');
  }
  ...
  /** Formats HTTP API ARN. */
  private resolveHttpApiArn(api: IHttpStage): string {
    const region = Stack.of(api).region;
    const apiId = api.api.apiId;
